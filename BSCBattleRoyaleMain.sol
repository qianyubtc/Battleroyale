...
contract BSCBattleRoyale {
    ...
    event GameFinished(uint256 indexed gameId, uint8 survivingRoom, uint256 totalPot, uint256 feeTaken, uint256 distributed, bool survivingRoomEmpty);
    ...
    function setMinDeposit(uint256 newMin) external onlyOwner {
        require(newMin > 0 && newMin <= 1 ether, "Invalid");
    ...
    function _settleGame(uint256 gid) internal {
        ...
        (uint8 survivingRoom, uint256 addrEntropy, uint256 seedVal) =
            _pickSurvivor(gid, totalPot, roomTotals);

        (uint256 feeTaken, uint256 distributed, bool empty) =
            _distributeRewards(gid, survivingRoom, totalPot);

        uint256 idx = gameHistory.length;
        gameIdToIndex[gid] = idx;

        _recordHistory(gid, survivingRoom, totalPot, feeTaken, distributed,
                       roomTotals, playerCounts, addrEntropy, seedVal);

        emit GameFinished(gid, survivingRoom, totalPot, feeTaken, distributed, empty);
    }
    ...
    function _pickSurvivor(
        uint256           gid,
        uint256           totalPot,
        uint256[4] memory roomTotals
    ) internal view returns (uint8 survivingRoom, uint256 addrEntropy, uint256 seedVal) {
        for (uint8 i = 0; i < 4; i++) {
        ...
        survivingRoom = active[seedVal % cnt];
    }
    ...
    function _distributeRewards(
        ...
        uint256 winnerTotal = gameRooms[gid][survivingRoom].totalAmount;
        if (winnerTotal == 0) {
            ...
            _payWinners(gid, survivingRoom, distributed, winnerTotal);
        }
    }
    ...
    function _payWinners(
        uint256 gid, uint8 survivingRoom, uint256 distributed, uint256 winnerTotal
    ) internal {
        address[] storage players = gameRooms[gid][survivingRoom].players;
        uint256 len       = players.length;
        uint256 totalPaid = 0;
        for (uint256 j = 0; j < len; j++) {
            address player  = players[j];
            uint256 deposit = gameRooms[gid][survivingRoom].deposits[player];
            uint256 payout  = (j == len - 1)
                ? distributed - totalPaid
                : (distributed * deposit) / winnerTotal;
            if (payout == 0) continue;
            totalPaid += payout;
            playerStats[player].totalWon += payout;
            (bool ok,) = payable(player).call{value: payout}("");
            if (ok) {
                emit WinnerPaid(gid, player, payout);
            } else {
                claimable[player] += payout;
                emit WinnerPayFailed(gid, player, payout);
            }
        }
    }

    function _recordHistory(
        uint256 gid, uint8 survivingRoom, uint256 totalPot,
        uint256 feeTaken, uint256 distributed,
        uint256[4] memory roomTotals, uint256[4] memory playerCounts,
        uint256 addrEntropy, uint256 seedVal
    ) internal {
        gameHistory.push(GameResult({
            gameId:            gid,
            survivingRoom:     survivingRoom,
            totalPot:          totalPot,
            feeTaken:          feeTaken,
